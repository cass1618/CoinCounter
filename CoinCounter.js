const calculateChange = (amt, i = 0, changeArray = []) => {
  const coinValues = [25, 10, 05, 01]
  if (amt > 0) {
    if (amt >= coinValues[i]) {
    ...
    const nickelsArray = changeArray.filter(e => e === 05)
    const penniesArray = changeArray.filter(e => e === 01)
